...
**Important caveat:** the within-(k) correlations are computed from (n=2) (two models per (k)). A correlation from two points is always (\pm 1); the −0.943 is an average of such values across (k). Treat this as sign information, not as a meaningful correlation coefficient. The delta table above is the real evidence.
...
Here SSE/token = mse_mean × d_model and SST/token is backed out from (R^2) via (R^2 = 1 - \mathrm{SSE}/\mathrm{SST}). The 410M mid layer has about 18× larger mean-centered variance scale (SST/token), so variance explained can look great even when absolute errors are not small.
